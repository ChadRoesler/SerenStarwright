#!/bin/bash
# ══════════════════════════════════════════════════════════════
# seren-setup.sh — Unified Jetson Seren Stack Installer
#
# Dispatcher script. Detects platform (Xavier/Nano), parses flags,
# does preflight + sudoers + hostname, then sources the appropriate
# platform module to run prereq + service phases.
#
# Modular layout:
#   seren-setup.sh           ← you are here
...
#
# Usage:
#   bash seren-setup.sh [SERVICE FLAGS] [OPTIONS]
#
# Service flags:
...
#   bash seren-setup.sh -l -k -d
#
#   # Xavier 16GB specialist — comfy only
#   bash seren-setup.sh -c
#
#   # Nano edge — everything plus Coral TPU
#   bash seren-setup.sh --all --coral
#
#   # Pin to a specific release
#   bash seren-setup.sh -l -k -d --tag 2026.04.29-xavier
# ══════════════════════════════════════════════════════════════
...
Options:
  ...
  -H, --hostname NAME   Hostname (default: auto-derived from services)
      ...
      --no-max-power    Skip MAXN power mode + jetson_clocks (default: ON).
                        Use on passively-cooled or battery-powered Jetsons.
  -h, --help            Show this help
...
if [ ! -d "$PLATFORM_DIR" ]; then
    ...
    fail "Did you extract the full seren-setup tarball?"
    exit 1
fi
...
# Generated by seren-setup.sh for user: $TARGET_USER
