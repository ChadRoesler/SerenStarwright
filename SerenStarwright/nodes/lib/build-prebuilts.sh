...
build_python() {
    ...
    if [ -f /usr/local/lib/libsqlite3.so ] && [ -f /usr/local/include/sqlite3.h ]; then
        ...
        SQLITE_HINT="LDFLAGS=-L/usr/local/lib -Wl,-rpath,/usr/local/lib CPPFLAGS=-I/usr/local/include"
    else
    ...
    env $SQLITE_HINT ./configure --enable-optimizations --prefix=/usr/local
...
# SQLite must run BEFORE Python in the same pipeline so build_python can
# link against the modern libsqlite3 in /usr/local. This is why --sqlite is
# dispatched first even though build_python is defined first in this file.
