...
$dbInstance = $Instance
$storePath = if ($layout.Data) { "'$($layout.Data)\chroma'" } else { "~/.seren-memory$dbInstance/chroma" }
@"
...
storage:
  persist_dir: $storePath$(if ($Corp) {"`ntls:`n  trust_system_store: true"})
...
if ($Service) { Setup-Autostart -ScriptDir $ScriptDir -ServiceName "seren-memory" -AppDir $AppDir -Token $Token -VenvDir $VenvDir -ServiceUser $ServiceUser -LocalSystem:$LocalSystem }
