...
$layout  = Get-SerenLayout -Root $Root -Short "margin" -Instance $Instance -VenvDir $VenvDir -AppDir "$env:USERPROFILE\seren-margin"
$VenvDir = $layout.Venv
$AppDir  = $layout.App
...
$dbInstance = $Instance
$storePath = if ($layout.Data) { "'$($layout.Data)\notes.db'" } else { "~/.seren-margin$dbInstance/notes.db" }
@"
