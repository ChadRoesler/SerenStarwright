...
if ($Service) {
  ...
  if ($Token) { "$Token" | Write-SerenTextFile -Path "$AppDir\seren-lodestar.env" }
  Setup-Autostart -ScriptDir $ScriptDir -ServiceName "seren-lodestar" -AppDir $AppDir -Token $Token -VenvDir $VenvDir -ServiceUser $ServiceUser -LocalSystem:$LocalSystem
}
