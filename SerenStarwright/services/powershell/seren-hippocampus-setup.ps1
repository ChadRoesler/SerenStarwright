...
if (-not $MemoryToken) { Warn "No -MemoryToken: fine if Memory has no bearer; a Memory installed with -GenToken answers 401 to every sleep." }
...
$memoryToken = if ($MemoryToken) { "  bearer_token: `"$MemoryToken`"`n" } else { "" }
@"
