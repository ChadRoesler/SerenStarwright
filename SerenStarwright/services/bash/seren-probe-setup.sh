...
# Storage: SerenProbe keeps topology state, eval results and corpus captures
# under ~/.seren-probe/ unless storage.state_dir says otherwise. Under an
# install root it says the root's store, so two clusters' probes on one host
# never share results.
STORAGE_BLOCK=""
[[ -n "$DATA_DIR" ]] && STORAGE_BLOCK=$'\nstorage:\n  state_dir: \''"$DATA_DIR"$'\''
cat > "$CFG_PATH" <<YAML
...
$INSTALL_SERVICE && setup_autostart "$SCRIPT_DIR" "seren-probe" "$APP_DIR" "$TOKEN" "$SVC_SUFFIX" "$VENV_DIR" "$SERVICE_USER"
