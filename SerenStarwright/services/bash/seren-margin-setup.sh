...
#    --token TOKEN    Set a bearer token (optional; Margin never requires one)
...
$INSTALL_SERVICE && setup_autostart "$SCRIPT_DIR" "seren-margin" "$APP_DIR" "$TOKEN" "$INSTANCE" "$VENV_DIR" "$SERVICE_USER"
