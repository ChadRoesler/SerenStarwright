...
# A URL says where. A CONFIG PATH says where AND presents that service's
# bearer, read from its own config so the token never crosses a command line.
# Starwright hands a setup's members to each other this way.
...
# The stores: a sibling's config gives its url AND its bearer, in its own words.
MEMORY_TOKEN_LINES=""; LOCI_TOKEN_LINES=""
if [[ -n "$MEMORY_CONFIG" ]] && seren_read_sibling_config "$MEMORY_CONFIG"; then
  ...
  [[ -n "$MEMORY_TOKEN_LINES" ]] && MEMORY_TOKEN_LINES="$MEMORY_TOKEN_LINES"$'\n'
  ok "Memory: ${MEMORY_URL} (from $MEMORY_CONFIG$([[ -n "$MEMORY_TOKEN_LINES" ]] && echo ", with its bearer"))"
fi
if [[ -n "$LOCI_CONFIG" ]] && seren_read_sibling_config "$LOCI_CONFIG"; then
  [[ -n "$SIB_URL" ]] && LOCI_URL="$SIB_URL"
  LOCI_TOKEN_LINES="$(seren_sibling_token_lines "      ")"
  [[ -n "$LOCI_TOKEN_LINES" ]] && LOCI_TOKEN_LINES="$LOCI_TOKEN_LINES"$'\n'
  ok "Loci: ${LOCI_URL} (from $LOCI_CONFIG$([[ -n "$LOCI_TOKEN_LINES" ]] && echo ", with its bearer"))"
fi
...
federation:
  stores:
    - name: memory
      type: seren_memory
      url: ${MEMORY_URL}
${MEMORY_TOKEN_LINES}    - name: loci
      type: seren_loci
      url: ${LOCI_URL}
${LOCI_TOKEN_LINES}$( $CORP && printf '\ntls:\n  trust_system_store: true\n' )
YAML
[[ -n "$TOKEN" || -n "$MEMORY_TOKEN_LINES" || -n "$LOCI_TOKEN_LINES" ]] && chmod 600 "$CFG_PATH"
