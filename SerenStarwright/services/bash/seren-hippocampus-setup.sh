...
[[ -z "$MEMORY_TOKEN" ]] && warn "No --memory-token: fine if Memory has no bearer; a Memory installed with --gen-token will answer 401 to every sleep."
...
memory:
  url: ${MEMORY_URL}
$( [[ -n "$MEMORY_TOKEN" ]] && printf '  bearer_token: "%s"\n' "$MEMORY_TOKEN" )
...
YAML
[[ -n "$TOKEN" || -n "$MEMORY_TOKEN" ]] && chmod 600 "$CFG_PATH"
