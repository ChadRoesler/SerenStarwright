...
# ~/.seren/secrets.json ({"observatory_token": "..."}, chmod 600), written by
# this installer when --token / --gen-token is given.
server:
  host: ${HOST}
  port: ${PORT}
YAML
...
# and hand it only to the launcher's env file - which the observatory never
# reads (auth.load_token opens ~/.seren/secrets.json and nothing else) - and
# then tell you to run a seren-secrets.sh that does not exist in this tree.
# So every install fell closed on its management endpoints, forever, while
# printing a token. Written here, merged into the file if one exists.
if [[ -n "$TOKEN" ]]; then
  SECRETS_HOME="$HOME"
  if [[ -n "$SERVICE_USER" && "$SERVICE_USER" != "$(id -un)" ]]; then
    SECRETS_HOME="$(getent passwd "$SERVICE_USER" 2>/dev/null | cut -d: -f6)"
    [[ -n "$SECRETS_HOME" ]] || SECRETS_HOME="/home/$SERVICE_USER"
...
PY
  ...
  if [[ "$SECRETS_HOME" == "$HOME" ]]; then
    _write_secret "$SECRETS_FILE" "$TOKEN"
  else
    sudo -u "$SERVICE_USER" env VPY="$VPY" bash -c "$(declare -f _write_secret); _write_secret '$SECRETS_FILE' '$TOKEN'"       || warn "could not write $SECRETS_FILE as $SERVICE_USER - write it by hand: {\"observatory_token\": \"...\"}"
  fi
  ok "Token written to $SECRETS_FILE (0600) - the management endpoints are armed"
fi
...
if [[ -n "$TOKEN" ]]; then
  echo -e "  Bearer token:    ${Y}${TOKEN}${NC}   (in ${SECRETS_FILE:-~/.seren/secrets.json})"
else
  echo -e "  ${Y}No token: the observatory FAILS CLOSED on start/stop/reboot until one exists.${NC}"
  echo -e "  ${Y}Re-run with --gen-token, or write ~/.seren/secrets.json: {\"observatory_token\": \"...\"}${NC}"
fi
